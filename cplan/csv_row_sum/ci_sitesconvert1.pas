...
procedure ConvertCSVFile(const sCSVFileName : string);
var
        ...
        repeat
              ...
              if (iElement = 1) then
                 sSiteKey := sElement
                  ...
                  rRowTotal := rRowTotal + StrToFloat(sElement);

              //if (sElement <> '') and (sElement <> ' ') then
              //   writeln(OutFile,sElement);
        until (iElement >= iElements);

        writeln(OutFile,sSiteKey + ',' + FloatToStr(rRowTotal));
   end;
...
begin
     try
        ...
        assignfile(OutFile,ExtractFilePath(sCSVFileName) + '\RowTotals.csv');
        rewrite(OutFile);
        writeln(OutFile,'SITEKEY,RowTotal');
...
begin
     ...
     begin
          ConvertCSVFile(EditInSites.Text);
